  ...
  return (
    ...
    >
      ...
      <div className="p-2 sm:p-2.5 flex-1 flex flex-col justify-between">
        <div>
          {product.subcategory && (
            <span className="text-[7.5px] sm:text-[8px] text-slate-400 uppercase font-bold tracking-wider leading-none block truncate mb-0.5">
              {product.subcategory}
            </span>
          )}
          <h4 className="text-[11px] sm:text-xs font-bold text-slate-800 line-clamp-1 group-hover:text-emerald-600 transition-colors leading-tight">
            {lang === "bn" ? product.nameBn : product.nameEn}
          </h4>
          <p className="text-[8.5px] sm:text-[9px] text-slate-400 line-clamp-1 leading-none mt-0.5">
            {lang === "bn" ? product.nameEn : product.nameBn}
          </p>
          
          <div className="flex items-center justify-between mt-1 text-[8.5px] sm:text-[9px]">
            ...
            {(product.unitBn || product.unitEn) && (
              <span className="text-[8px] sm:text-[8.5px] text-slate-400 font-medium truncate max-w-[50%]">
        ...
        <div className="mt-1.5 pt-1 border-t border-slate-100">
          <div className="flex items-baseline gap-1 flex-wrap">
            ...
            {product.originalPrice && product.originalPrice > product.price ? (
              <span className="text-[8.5px] sm:text-[9px] text-slate-400 line-through leading-tight">৳{fmtNum(product.originalPrice)}</span>
            ) : null}
          </div>
          ...
          <div className="grid grid-cols-2 gap-1 mt-1.5">
            <button 
              ...
              className={`py-1 px-1 text-[8px] sm:text-[9px] font-bold rounded-lg transition shadow-2xs cursor-pointer flex items-center justify-center whitespace-nowrap border ${
                isInCart 
            ...
            <button 
              ...
              className="py-1 px-1 text-[8px] sm:text-[9px] font-bold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 active:bg-emerald-800 transition shadow-2xs cursor-pointer flex items-center justify-center whitespace-nowrap"
            >
